	...
	<body>
		...
		<div id="show-reponse" class="content scaffold-show" role="main">
			...
			<ol class="property-list reponse">
				...
				<g:if test="${reponseInstance?.propostion}">
				<li class="fieldcontain">
					<span id="propostion-label" class="property-label"><g:message code="reponse.propostion.label" default="Propostion" /></span>
					
						<span class="property-value" aria-labelledby="propostion-label"><g:link controller="proposition" action="show" id="${reponseInstance?.propostion?.id}">${reponseInstance?.propostion?.encodeAsHTML()}</g:link></span>
