			...
			<table>
				<thead>
					<tr>
						...
						<th><g:message code="reponse.propostion.label" default="Propostion" /></th>
				...
				<g:each in="${reponseInstanceList}" status="i" var="reponseInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${reponseInstance.id}">${fieldValue(bean: reponseInstance, field: "user")}</g:link></td>
					
						<td>${fieldValue(bean: reponseInstance, field: "note")}</td>
					
						<td>${fieldValue(bean: reponseInstance, field: "commentaire")}</td>
					
						<td>${fieldValue(bean: reponseInstance, field: "propostion")}</td>
